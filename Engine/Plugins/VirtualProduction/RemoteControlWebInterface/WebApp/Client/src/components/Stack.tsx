...
import { ColorPicker, ColorPickerList, Button, Text, VectorDrawer, Dial, JoysticksWrapper, DialMode } from './controls';
import { PrecisionModal } from '../components';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { DraggableWidget, DroppableWidget } from '.';
...
export class Stack extends React.Component<Props & PropsFromState, State> {
  ...
  renderColorList = (widgets: ICustomStackProperty[], listIndex: number, path: string) => {
    ...
    let type = WidgetTypes.ColorPicker;

    if (first.widget === WidgetTypes.MiniColorPicker)
      type = WidgetTypes.MiniColorPicker;
          ...
          {list.items?.map((item, index) => {
            ...
            return (
              <div className="navigation-row" 
                   ...
                   data-value={itemId}
                   key={itemId}>
                <button className={`navigation-btn ${selected === item.id ? 'selected' : ''}`} onClick={() => onSelect(item.id)}><p>{_.startCase(item.label)}</p></button>
              </div>
            );
          })}
        </div>
      ...
      case IPanelType.Panel:
        ...
        return (
          <DraggableWidget draggableId={draggableId}
                           ...
                           isDragDisabled={!editable}>
            <DroppableWidget droppableId={droppableId}
                             className={`droppable-panel ${!!panel?.title ? 'has-title' : ''}`}
                             isDropDisabled={isDropDisabled}>
  ...
  render() {
    const { dragging, droppable, panels = [], tabKey, vector } = this.props;
    ...
    return (
      <div className={className}>
        <DroppableWidget droppableId={droppableId} className="droppable-root" isDropDisabled={isDropDisabled}>
