...
export class Snapshot extends React.Component<Props, State> {
  ...
  fetchAssets = async () => {
    let { map } = this.state;
    if (!map) {
      const world = await _api.proxy.function('/Script/EditorScriptingUtilities.Default__EditorLevelLibrary', 'GetEditorWorld', {});
      map = world.ReturnValue;
    }

    const { search, sortBy, sortAsc } = this.state;
    const assets = await _api.assets.search(search, ['LevelSnapshot', 'DisjunctiveNormalFormFilter'], '/Game', 200);
    let snapshots = _.filter(assets, asset => asset.Class === 'LevelSnapshot' && (!map || asset.Metadata?.MapPath === map));
    
    snapshots = this.sortAssets(snapshots, sortBy, sortAsc);
    const filters = _.filter(assets, asset => asset.Class === 'DisjunctiveNormalFormFilter');
...
class SnapshotPreview extends React.Component<SnapshotPreviewProps, SnapshotPreviewState> {
  ...
  onLoad = async () => {
    const { map, snapshot, onClose } = this.props;
