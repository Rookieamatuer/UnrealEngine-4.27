...
import { IPreset, IGroup, IExposedProperty, WidgetTypes, ICustomStackProperty, IPanel, ICustomStackTabs, IExposedFunction, 
        PropertyType, IView, ITab, TabLayout, ICustomStackWidget, WidgetType, IColorPickerList } from 'src/shared';
...
import { ColorPicker, Button, Text, Dial, JoysticksWrapper } from './controls';
...
export class PropertiesDrawer extends Component<Props, State> {
  ...
  onListItemRemove = async (i: number, key = 'items') => {
    const { selected, panels } = this.props;
    const [path, index] = selected.split('_');
    ...
    const items = _.get(panels, `${path}[${index}]${key}`, panels); 
    items.splice(i, 1);
                ...
                <Draggable draggableId={`REORDER_${item.id}`} index={i} key={item.id}>
                  {provided => (
                    <div ref={provided.innerRef}
                      ...
                      <FontAwesomeIcon className="icon" icon={['fas', 'times']} onClick={this.onListItemRemove.bind(this, i)} />
                    </div>
                  )}
                </Draggable>
              ))}
